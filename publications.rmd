...
uncounted_reviews <-
  c(
    ...
    "Plant Cell Reports 2024 [flagged for image manipulation; rejected]",
    "Scientific Reports 2024 [rejected]"
  ) |> 
  length()

co_first <- c(
  "https://doi.org/10.1002/advs.202414496", #2025 TOR PUP-IT
  "https://doi.org/10.1016/j.devcel.2025.01.021", #2025 ZFP2
  "https://doi.org/10.1016/j.molp.2022.12.015", #2022 MolPlant review
  "https://doi.org/10.1021/acssuschemeng.0c00194", #2020 ACS Raman 
  "https://doi.org/10.1093/plcell/koac284", #2022 PlantCell biomechanics
  "https://doi.org/10.1101/2021.06.12.447240" #2021 biorxiv biomechanics
  )

corresponding <- c(
  "https://doi.org/10.1021/acs.jafc.4c01999" #2024 JACF perspective
)
...
works <- tibble(
  ...
  alex_type = full_works[["results"]][["type"]],
  type = case_when(alex_type == "preprint" ~ "Preprint", authors_short == "L Blaschek" ~ "Editorial", TRUE ~ "Article"),
  # type = case_when(version == "submittedVersion" ~ "preprint", TRUE ~ "article"), # fall-back if built-in type fails
  ...
  mutate(
    item = case_when(
      doi %in% co_first ~ str_replace(item, short_name, paste0("<b>", short_name, "<sup>&#8224;</sup></b>")),
      doi %in% corresponding ~ str_replace(item, short_name, paste0("<b>", short_name, "*</b>")),
      type == "Editorial" ~ str_replace(item, short_name, paste0("<b>", short_name, "*</b>")),
      .default = str_replace(item, short_name, paste0("<b>", short_name, "</b>"))
      ),
    item = case_when(
      file.exists(paste0("assets/pdf/", str_replace_all(str_remove(doi, fixed("https://doi.org/")), "[\\./]", "_") , ".pdf")) ~
        paste0(item, " — [PDF](./assets/pdf/", str_replace_all(str_remove(doi, fixed("https://doi.org/")), "[\\./]", "_") , ".pdf)"),
      TRUE ~ item),
    item = case_when(
      doi == "https://doi.org/10.1021/acs.jafc.4c01999" ~ paste0(item, " | [Reproducible manuscript](./assets/2024_JAFC/index.html)"),
      TRUE ~ item
    )) |>
  arrange(date) |>
  drop_na(doi) |>
...
metrics <- tibble(
  ...
  value = c(
    ...
    h,
    i10,
    max(work_timeline$works_cum[work_timeline$type == "Article"]),
    ...
    sum(work_timeline$works[cite_timeline$year >= year(now()) - 5 & work_timeline$type == "Article"]),
...
kable(
  work_table |> filter(type == "Article") |> select(-type),
  "html",
  escape = FALSE
) |>
  kable_styling(
...
kable(
  work_table |> filter(type == "Editorial") |> select(-type),
  "html",
  escape = FALSE
) |>
  kable_styling(
...
work_plot <- ggplot(
  work_timeline |> filter(type %in% c("Article", "Preprint")),
  aes(
    x = pub_year,
    y = works_cum,
    colour = type
